 ...
 *
 * 目的：將目前存儲在 localStorage 的人員指派資料遷移到資料庫，
 * 提供更可靠的資料存儲和查詢功能
 *
 ...
 * @since 2025-09-24
 */
class CreatePersonnelAssignmentTables extends Migration
{
    /**
    ...
    public function up()
    {
        ...
        // 設定外鍵約束
        $this->forge->addForeignKey('company_id', 'local_companies', 'id', 'CASCADE', 'CASCADE');
        ...
        $this->forge->addForeignKey('personnel_id', 'external_personnel', 'id', 'CASCADE', 'CASCADE');
        ...
        if ($this->db->tableExists('activity_logs')) {
            $this->db->query("
                ...
                VALUES (
                    'SCHEMA_MIGRATION',
                    'DATABASE',
                    '建立人員指派管理表格 - 新增 external_personnel, personnel_assignments 表',
                    NOW()
                )
            ");
        }
        ...
        if ($this->db->tableExists('activity_logs')) {
            $this->db->query("
                ...
                VALUES (
                    'SCHEMA_ROLLBACK',
                    'DATABASE',
                    '回滾人員指派管理表格 - 移除 external_personnel, personnel_assignments 表',
                    NOW()
                )
            ");
        }
